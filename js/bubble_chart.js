...
function createBubbleChart() {
    ...
    function createNodes(rawData) {
        ...
        // UPDATA:更改了id值，添加了index
        var myNodes = rawData.map(function (data_row,index) {
            node = {
                id: index,
    ...
    function showLabels(mode) {
        ...
        var text = bubble_group_labels.enter().append('text')
            .attr('class', 'bubble_group_label')
            .attr('x', function (d) { return mode.gridCenters[d].x; })
            .attr('y', function (d) { return mode.gridCenters[d].y - grid_element_half_height + 5; })
            ...
            // .text(function (d) { return mode.texts[d] ? mode.texts[d] : d; });
            .selectAll('tspan')
            .data(function(d){
                var data = [];
                var text = mode.texts[d] ? mode.texts[d] : d;
                text = text.split(',');
                text.map(function(D){
                    var txt = D;
                    var list = {
                        text:txt,
                        x:mode.gridCenters[d].x
                    }
                    data.push(list);
                })
                return data;
        ...
        for (var key in currentMode.gridCenters) {
            if (currentMode.gridCenters.hasOwnProperty(key)) {
                var rectangle = inner_svg.append("rect")
                    ...
                    .attr("y", currentMode.gridCenters[key].y - grid_element_half_height - 20)
                    .attr("width", grid_element_half_width*2 - 10)
                    .attr("height", grid_element_half_height*2 - 10)
                    .attr("stroke", "gray")
					.attr("stroke-width", 1)
    ...
    function showTooltip(d) {
        ...
        d3.select(this)
          .selectAll('text')
          .attr('stroke', BUBBLE_PARAMETERS.bubble_hover_text_color)
          .attr('fill', BUBBLE_PARAMETERS.bubble_hover_text_color);
    ...
    function hideTooltip(d) {
        ...
        // var originalColor = d3.rgb('black').darker()
        // var originalColor = d3.rgb(fillColorScale(d.fill_color_group)).darker()
        // d3.select(this).attr('stroke', originalColor);
        d3.select(this)
          .selectAll('text')
          .attr('fill', BUBBLE_PARAMETERS.bubble_text_color)
          .attr('stroke', BUBBLE_PARAMETERS.bubble_text_color);
    ...
    function showAxis(mode) {
        ...
        //UPDATA:画x坐标轴,要将文字和线变成白色，css中更改的
        inner_svg.append("g")
            .attr("class", "axis axis--x")
            .attr("transform", "translate(0," + height + ")")
            .call(d3.axisBottom(xAxis))
        // UPDATA：x轴的文字标题，将标题放在了最左侧
...
*/
    ...
    function createBubbles() {
        ...
        // UPDATA:新建节点，将圆圈、文字分为一组
        node = inner_svg.selectAll('.node')
        ...
        node.append('circle').attr('r', 0) // Initially, their radius (r attribute) will be 0.
        ...
        //UPDATA:给bubble中间添加文字，太小的就不添加
        //截切路线，使绑定显示的元素不超过此设定区域。XLink：href是绑定到指定的元素上
        node.append("clipPath")
            .attr("id", function(d) { return "clip-" + d.id; })
            .append("use")
            .attr("xlink:href", function(d) { return "#" + d.id; });

        // node.filter(function(d){ return d["NO OF RECORDS STOLEN"]>20000000;})
        // d.scaled_radius < 40 ? d.scaled_radius < 35 ? '.5em' : '.7em' : '1em'

        node.filter(function(d){ return d["NO OF RECORDS STOLEN"]>10000000;})
            .append("text")
            .attr("clip-path", function(d) { return "url(#clip-" + d.id + ")"; })
            .style("font-size",function(d) { return d.scaled_radius * 0.25 + 2 + 'px';})
            // .style("font-size",function(d) { return d.scaled_radius * 0.2 + 3 + 'px';})
            // .attr("stroke",function(d){return d3.rgb(fillColorScale(d.fill_color_group)).darker();})
            .attr("stroke",BUBBLE_PARAMETERS.bubble_text_color)
            .attr("fill",BUBBLE_PARAMETERS.bubble_text_color)
            .selectAll("tspan")
            .data(function(d) { return d.Entity.split(' '); })
            .enter().append("tspan")
            .attr("x", 0)
            .attr("y", function(d, i, nodes) { return 18 + (i - nodes.length / 2 - 0.5) * 12;})
        ...
        if (!isStatic) {
            ...
            if(BUBBLE_PARAMETERS.force_type == "collide") {
                var bubbleCollideForce = d3.forceCollide()
                    .radius(function(d) { return d.scaled_radius + 2; })
                    .iterations(8)
                forceSim
            ...
            if(BUBBLE_PARAMETERS.force_type == "charge") {
                ...
                // UPDATA:bubble之间的力
                var bubbleDistanceMax = currentMode.gridDimensions.rows > 1 ? 250 : 550 ;

                function bubbleCharge(d) {
                    return -Math.pow(d.scaled_radius, 2.0) * 0.8 * (+BUBBLE_PARAMETERS.force_strength);
                }

                forceSim
                    .force('charge', d3.forceManyBody().strength(bubbleCharge).distanceMax(bubbleDistanceMax));
    ...
    var bubbleChart = function bubbleChart(parentDOMElement, rawData) {
        ...
        radiusScale = d3.scalePow()
            .exponent(0.5)
            .range([5, 130])  // Range between 2 and 25 pixels
    ...
    bubbleChart.switchMode = function (buttonID) {
        ...
        // SHOW LABELS (if we have more than one category to label)&& currentMode.size > 1
        ...
        var targetForceX = d3.forceX(function(d) {return targetFunction(d).x})
            .strength(+BUBBLE_PARAMETERS.force_strength*0.9);
        var targetForceY = d3.forceY(function(d) {return targetFunction(d).y})
            .strength(+BUBBLE_PARAMETERS.force_strength*1.1);
        ...
        forceSim.alphaTarget(2).restart();

        clearTimeout(forceTimer);
        forceTimer = setTimeout(function(){
            if (forceSim) {
                // Stop any forces currently in progress
                console.log('forceSim')
                forceSim.stop();
            }
        },18000);
    };
...
function ViewMode(button_id, width, height) {
    ...
    if (this.type == "grid") {
        ...
        for(var i=0; i<this.size; i++) {
            ...
            var currentCenter = {
                x: (2 * cur_col + 1) * (width / (this.gridDimensions.columns * 2))-(this.size > 1 ? 0 : 80),   //UPDATA:显示全部往左移看起来在中间
                y: (2 * cur_row + 1) * (height / (this.gridDimensions.rows * 2))
            };

            this.gridCenters[this.labels[i]] = currentCenter;
            if(curMode.texts)
             this.texts[this.labels[i]] = curMode.texts[i];
        }
    }
...
var sensitivityArrow = document.getElementById('sensitivityArrow');

// Create a new bubble chart instance
var myBubbleChart = createBubbleChart();

// Load data
// UPDATA:修改数据为不请求文件，直接写出js对象
...
// 以上注释内容更改为：
// Display bubble chart inside the #vis div.
myBubbleChart('#vis', BUBBLE_PARAMETERS.datas);
...
function setupButtons() {
    ...
    d3.select('#toolbar')
        ...
        .on('click', function () {
            ...
            // if (buttonId == 'sensititity') {
            //     sensitivityArrow.style.opacity = 1;
            // }else{
            //     sensitivityArrow.style.opacity = 0;
            // }
